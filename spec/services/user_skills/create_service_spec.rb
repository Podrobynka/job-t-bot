...
RSpec.describe UserSkills::CreateService, :aggregate_failures do
  ...
  context 'when user_skill exists' do
    ...
    it 'finds existed user_skill' do
  ...
  context 'when user_skill doesn\'t exist' do
    it 'creates new user_skill' do
      expect { subject }.to change { UserSkill.count }.by(1)
      expect(subject).to eq(UserSkill.last)
    end
  end
end
